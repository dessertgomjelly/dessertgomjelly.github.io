...
<img src="{{site.url}}/images/2024-04-01-컴퓨터네트워크/Untitled 1.png" style="zoom:90%;" />
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/87052024-dbf2-449a-978f-6752bea53f15/a33c0c4f-a70f-4d82-ac90-429f919119a9/Untitled.png)
...
-  모든 packet switch는 packet을 수신하고 난 다음에 전달
   ...
   ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/87052024-dbf2-449a-978f-6752bea53f15/9c3aa34c-6de3-44dd-a8f9-ec87824e52b0/Untitled.png)
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/87052024-dbf2-449a-978f-6752bea53f15/e11d15af-900a-465a-bb4d-6137ab5806c9/Untitled.png)
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/87052024-dbf2-449a-978f-6752bea53f15/6948bd23-5d63-4362-9068-b14f62bf3995/Untitled.png)

**d_nodal (노드 지연시간)**: 노드 지연시간은 패킷이 노드(라우터 또는 스위치)를 통과하는 데 걸리는 총 시간을 의미합니다. 이는 처리 지연시간(d_proc), 큐 대기 지연시간(d_queue), 전송 지연시간(d_trans), 전파 지연시간(d_prop)의 합으로 나타낼 수 있습니다.

**d_proc (처리 지연시간)**: 처리 지연시간은 패킷이 노드에서 처리되는 데 걸리는 시간을 의미합니다. 이는 패킷이 노드에서 처리되는 과정에서 발생하는 지연시간으로, 라우팅, 버퍼링, 검사 등의 작업으로 인해 발생합니다.

**d_queue (큐 대기 지연시간)**: 큐 대기 지연시간은 패킷이 노드의 출력 큐에서 대기하는 시간을 의미합니다. 이는 다른 패킷이 노드를 통과하고 있는 동안 패킷이 대기해야 하는 시간으로, 출력 링크의 혼잡도에 따라 변할 수 있습니다.

**d_trans (전송 지연시간)**: 전송 지연시간은 패킷이 전송 매체를 통해 전송되는 데 걸리는 시간을 의미합니다. 이는 물리적인 매체를 통해 데이터가 전송되는 시간으로, 전송 매체의 대역폭과 속도에 따라 결정됩니다.

**d_prop (전파 지연시간)**: 전파 지연시간은 데이터가 전송 매체를 통해 전파되는 데 걸리는 시간을 의미합니다. 이는 전파 속도와 거리에 따라 결정되며, 신호가 발생한 위치에서 목적지까지의 거리에 비례합니다.
